# Diagnostic: verify DAOs & Funding lock effectiveness for a specific user.
#
...
#   2. Whether Discourse's Guardian thinks they can create a topic in DAOs & Funding
#   3. The category_groups rows in DB for DAOs & Funding (the actual lock state)
#
...
dp = Category.find_by(name: "DAOs & Funding", parent_category_id: nil)
abort "DAOs & Funding category not found" unless dp
...
all_categories = [dp] + Category.where(parent_category_id: dp.id).order(:id).to_a

all_categories.each do |cat|
  label = cat.id == dp.id ? "DAOs & Funding (top-level)" : "DAOs & Funding > #{cat.name}"
...
subcats = Category.where(parent_category_id: dp.id).order(:id).to_a
...
if user_restricted
  ...
  puts "  WARNING: 'everyone -> create_post' row missing on DAOs & Funding top-level. Lock NOT applied."
...
elsif g.can_create_topic_on_category?(dp)
